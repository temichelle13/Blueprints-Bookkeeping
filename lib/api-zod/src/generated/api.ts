...
export const UnsubscribeNewsletterByTokenQueryParams = zod.object({
  "token": zod.string().uuid().describe('The unique unsubscribe token for the subscriber')
})
...
export const ListContractsResponseItem = zod.object({
  ...
  "status": zod.enum(['draft', 'sent', 'viewed', 'signed', 'expired', 'cancelled']),
...
export const CalWebhookBody = zod.object({
  "triggerEvent": zod.enum(['BOOKING_CREATED', 'BOOKING_RESCHEDULED', 'BOOKING_CANCELLED']),
  "payload": zod.object({
...
export const GetNexusNotificationsResponse = zod.array(GetNexusNotificationsResponseItem)
