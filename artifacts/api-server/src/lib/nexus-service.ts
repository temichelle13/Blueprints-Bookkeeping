...
export async function getNexusSummary(): Promise<StateNexusSummary[]> {
  ...
  return rules.map((rule) => {
    ...
    const riskLevel = computeRiskLevel(
      clientCount,
      rule.foreignQualificationThreshold,
...
export async function runNexusCheck(): Promise<{
  warnings: number;
  alerts: number;
}> {
...
async function sendNexusNotification(
  ...
  try {
    ...
    console.error(
      `Failed to send nexus notification for ${rule.stateCode}:`,
      err,
    );
    return false;
  }
